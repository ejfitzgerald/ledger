...
BlockCoordinator::State BlockCoordinator::OnSynchronised(State current, State previous)
{
  ...
  info_.Reset();
  info_.Start("00 Full Block lifecycle");
...
BlockCoordinator::State BlockCoordinator::OnTransmitBlock()
{
  ...
  try
  {
    ...
    if (BlockStatus::ADDED == chain_.AddBlock(*next_block_))
    {
      ...
      info_.Stop("07 Transmit");
      info_.Stop("00 Full Block lifecycle");
      info_.Print();

      static uint64_t total_txs_sent = 0;

      total_txs_sent += next_block_->GetTransactionCount();

      FETCH_LOG_INFO(LOGGING_NAME, "Total TXs sent: ", total_txs_sent);
      ...
      if(next_block_->GetTransactionCount() == 0)
      {
        FETCH_LOG_INFO(LOGGING_NAME, "Waiting 1 minute to allow TXs to accumulate");
        state_machine_->Delay(std::chrono::seconds{120});
      }
    }
    else
    {
