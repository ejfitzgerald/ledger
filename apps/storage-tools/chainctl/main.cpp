...
//      ConstByteArray{"248b38fd6a97e13345a15ba40737d2d2ad64d99f943cc7b981718c11c2b5e096"}.FromHex();
  ...
  cfg.fast_load = false; // unsafe!
  MainChain chain{MainChain::Mode::LOAD_PERSISTENT_DB, cfg};

  auto heaviest = chain.GetHeaviestBlockHash();
  std::cout << "Heaviest: 0x" << heaviest.ToHex() << std::endl;

  auto block = chain.GetHeaviestBlock();
  if (block)
  {
    std::cout << "Heaviest: #" << block->block_number << " 0x" << block->hash.ToHex() << std::endl;
  }

  block = chain.GetBlock(ConstByteArray{"2662b1ab5e321876f9836e8ad93c175048897e472a14617e1d71df72713b4147"}.FromHex());
  if (block)
  {
    std::cout << "Heaviest: #" << block->block_number << " 0x" << block->hash.ToHex() << std::endl;
  }
